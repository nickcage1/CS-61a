...
def balanced(m):
    ...
    if left_torque != right_torque:
        return False
    elif is_mobile(end(right(m))) == False and is_mobile(end(left(m))) == False:
        return True
    else:
        sum = [balanced(end(right(m))) , balanced(end(left(m)))]
        if False in sum:
            return False
        return True
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        pee = [totals_tree(end(right(m)))]
        poop = [totals_tree(end(left(m)))]
        return tree(total_weight(m),poop + pee)
...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t):
        ...
        sum = [replace_leaf(x , find_value, replace_value) for x in branches(t)]
        return tree(label(t) , sum)
...
def has_path(t, word):
    ...
    if label(t)[0] != word[0]:
        ...
        if label(t)[0] == word:
            return True
    sum = [has_path(x,word[1:]) for x in branches(t)]
    return True in sum
...
def check_par():
    ...
    True
    """
    r1 = interval(2, 5) # Replace this line!
